...
center&&(wrapper.style.position="absolute",wrapper.style.left=parseInt((w-newWidth)/2)+"px",wrapper.style.top=parseInt((h-newHeight)/2)+"px")},paper.scaleAll=function(amount){paper.changeSize(width*amount,height*amount)},paper.changeSize(width,height),paper.w=width,paper.h=height,paper}}),define("parccTei/portableLib/pointFactory",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash"],function($,_){"use strict";function pointFactory(paper,grid,options){options=_.defaults(options||{},_defaults)
;var _color=options.color,_r=parseInt(options.radius),_x=0,_y=1,_label="",_rGlow=parseInt(options.glowRadius)||3*_r,_events=options.on||{},_dragEnabled=!1,obj={children:paper.set(),uid:_.uniqueId(),setColor:function(color){_color=String(color)},setX:function(val){_x=parseInt(val)},getX:function(){return _x},getLabel:function(){return _label},getY:function(){return _y},setY:function(val){_y=parseInt(val)},setLabel:function(val){_label=val},setCartesianCoord:function(x,y){_x=grid.getOriginPosition().left+grid.getUnits().x*x,_y=grid.getOriginPosition().top-grid.getUnits().y*y},getCartesianCoord:function(){
var cartesianX=(_x-grid.getOriginPosition().left)/grid.getUnits().x,cartesianY=-(_y-grid.getOriginPosition().top)/grid.getUnits().y;return{x:Number(cartesianX.toFixed(2)),y:Number(cartesianY.toFixed(2))}},setCoord:function(x,y){var coord=grid.snap(parseInt(x),parseInt(y));_x=coord[0],_y=coord[1]},setR:function(val){_r=parseInt(val)},setRGlow:function(val){_rGlow=parseInt(val)},render:function(){this.remove();var circle=paper.circle(_x,_y,_r);if(options.fill?circle.attr({fill:_color,stroke:"#000",cursor:"move"}):circle.attr({stroke:_color,"stroke-width":3,cursor:"move"}),this.children.push(circle),options.glow&&this.showGlow(),_label){
...
cursor:"move"});this.children.length>1&&this.children.pop().remove(),this.children.push(glow),this.children.attr("cursor","move")},hideGlow:function(){this.children.length>1&&this.children.pop().remove(),this.children.attr("cursor","default")},setOption:function(key,value){options[key]=value}};return options.cartesian?obj.setCartesianCoord(options.x,options.y):obj.setCoord(options.x,options.y),obj.setLabel(options.label),obj}var _defaults={x:0,y:0,color:"#f00",radius:10,glow:!0,glowRadius:0,glowOpacity:.3,fill:!0,removable:!0,cartesian:!1};return pointFactory}),define("parccTei/portableLib/axisFactory",["taoQtiItem/portableLib/lodash"],function(_){
"use strict";function getArrowsPath(orientation,config){var tipPosition,arrowConfig={length:.7*config.unitSize,tip:.3*config.unitSize,orientation:orientation},axisSizePx=(config.max-config.min)*config.unitSize,path="";if("right"===arrowConfig.orientation)tipPosition=config.left+axisSizePx+arrowConfig.length,path+="M"+(config.left+axisSizePx)+","+config.top,path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition-arrowConfig.tip)+","+(config.top+config.divisionWidth),path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition-arrowConfig.tip)+","+(config.top-config.divisionWidth),path+="L"+tipPosition+","+config.top;else{
if("left"!==arrowConfig.orientation)throw"unknown orientation type to the arrow";tipPosition=config.left-arrowConfig.length,path+="M"+config.left+","+config.top,path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition+arrowConfig.tip)+","+(config.top+config.divisionWidth),path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition+arrowConfig.tip)+","+(config.top-config.divisionWidth),path+="L"+tipPosition+","+config.top}return path}function _applyStyle(path,config){path.attr({stroke:config.color?config.color:_defaults.color,"stroke-width":config.thickness?config.thickness:_defaults.thickness,opacity:config.opacity?config.opacity:_defaults.opacity})}
function addCssClass(raphaelObj,newClass){var pattern=new RegExp("\\b"+newClass+"\\b"),oldClass=raphaelObj.node.getAttribute("class")||"";raphaelObj.node.setAttribute("class",pattern.test(oldClass)?oldClass:oldClass+" "+newClass)}function getLabel(i,config){return config&&_.isArray(config.labels)&&void 0!==config.labels[i]&&!1!==config.labels[i]?config.labels[i]:i}function axisFactory(paper,config){config=_.defaults(config||{},_defaults);var arrowRight,arrowLeft,set=paper.set(),steps=[],obj={getMin:function(){return config.min},getMax:function(){return config.max},getCanvas:function(){return paper},getOriginPosition:function(){return{
left:config.left-config.unitSize*config.min,top:config.top}},snap:function(x0){var step=config.unitSize/config.unitSubDivision;return[paper.raphael.snapTo(steps,x0,step/2),config.top]},render:function(){this.clear();var path="M"+config.left+","+config.top,axisSizePx=(config.max-config.min)*config.unitSize,subDivisionSize=config.unitSize/config.unitSubDivision;path+="L"+(config.left+axisSizePx)+","+config.top;var position=config.left;steps.push(position);for(var i=config.min;i<=config.max;i++){path+="M"+position+","+(config.top-config.divisionWidth),path+="L"+position+","+(config.top+config.divisionWidth)
;var label=paper.text(position,config.top-config.divisionWidth-config.fontSize/2-5,getLabel(i,config));if(label.attr({"font-size":config.fontSize}),addCssClass(label,"scene scene-text"),set.push(label),i<config.max){var subPosition=position+subDivisionSize;steps.push(subPosition);for(var j=config.unitSubDivision;j>1;j--)path+="M"+subPosition+","+(config.top-config.subDivisionWidth),path+="L"+subPosition+","+(config.top+config.subDivisionWidth),subPosition+=subDivisionSize,steps.push(subPosition)}position+=config.unitSize,steps.push(position)}config.arrows&&(arrowRight=this.buildArrow("right"),arrowLeft=this.buildArrow("left"));var pathObj=paper.path(path)
;addCssClass(pathObj,"scene scene-grid"),_applyStyle(pathObj,config),set.push(pathObj)},getSet:function(){return set},isRendered:function(){return set.length},clear:function(){set.length>0&&(steps=[],set.remove().clear())},clickable:function(){var borderBox=set.getBBox(),clickableArea=paper.rect(borderBox.x,borderBox.y,borderBox.width,borderBox.height);return clickableArea.attr({fill:"rgba(0,0,0,0)",stroke:0}),set.push(clickableArea),clickableArea},getWidth:function(){return set.getBBox().width},getHeight:function(){return set.getBBox().height},getUnits:function(){return{x:config.unitSize,y:0}},coordinateToPosition:function(x){return{
left:this.getOriginPosition().left+this.getUnits().x*x,top:this.getOriginPosition().top}},buildArrow:function(orientation){var pathStr=getArrowsPath(orientation,config),arrow=paper.path(pathStr);return addCssClass(arrow,"scene scene-grid"),_applyStyle(arrow,config),set.push(arrow),arrow},getConfig:function(){return _.clone(config)},setConfig:function(key,value){config[key]=value},buildContainerBox:function(options){var _defaults={padding:20,absolute:!0,shadow:!1};options=_.defaults(options||{},_defaults);var box,padding=options.padding,bb=set.getBBox();if(options.absolute)box=paper.rect(bb.x-padding,bb.y-padding,bb.width+2*padding,bb.height+2*padding);else{
var axisSizePx=(config.max-config.min)*config.unitSize;box=paper.rect(config.left-padding,config.top-padding,axisSizePx+2*padding,config.divisionWidth+2*padding)}return box.attr({fill:"#fff"}),set.push(box),box.insertBefore(set),options&&options.shadow&&set.push(box.glow({width:5,fill:!0,offsetx:3,offsety:3,color:"#999"})),box},addCssClass:addCssClass};return obj.render(),obj}var _defaults={top:40,left:40,min:-5,max:5,labels:!1,thickness:2,color:"#000",divisionWidth:16,subDivisionWidth:8,unitSubDivision:2,unitSize:50,fontSize:18,arrows:!1,opacity:1};return axisFactory}),
define("parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/libs/axisZoom",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function createZoomable(axis){function addRectangle(position,coord){var rect=paper.rect(position,top,subDivisionSize,2*config.divisionWidth);rect.attr({stroke:"#1a6596",fill:"#e6eef4"}),rect.hide(),clickableSet.push(rect),clickables.push({position:position,rect:rect,coord:coord,active:!1})}
for(var clickables=[],paper=axis.getCanvas(),config=axis.getConfig(),clickableSet=paper.set(),subDivisionSize=config.unitSize/config.unitSubDivision,subDivisionIncrement=1/config.unitSubDivision,left=config.left,top=config.top-config.divisionWidth,i=config.min;i<config.max;i++){if(addRectangle(left,i),i<config.max)for(var left_=left,i_=i,j=2;j<=config.unitSubDivision;j++)left_+=subDivisionSize,i_+=subDivisionIncrement,addRectangle(left_,i_);left+=config.unitSize}return clickableSet.insertBefore(axis.getSet()),clickables}function drawPath(paper,points){var pathStr="";return _.each(points,function(point){pathStr+=pathStr?"L":"M",pathStr+=point.toString()}),
pathStr+="Z",paper.path(pathStr).attr({stroke:"#1a6596",fill:"#e6eef4"})}function drawZoomEffect(paper,from,to){var source=from.getBBox(),target=to.getBBox(),points=[[source.x,source.y+source.height],[source.x+source.width,source.y+source.height],[target.x+target.width,target.y],[target.x,target.y]],zoomEffect=drawPath(paper,points);return zoomEffect&&(to.zoomEffect&&to.zoomEffect.remove(),to.zoomEffect=zoomEffect,zoomEffect.insertBefore(to)),zoomEffect}return{createZoomable:createZoomable,drawZoomEffect:drawZoomEffect}}),define("css!parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/css/graphZoomNumberLineInteraction",[],function(){}),
define("graphZoomNumberLineInteraction/runtime/graphZoomNumberLineInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","parccTei/portableLib/pointFactory","parccTei/portableLib/axisFactory","parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/libs/axisZoom","css!parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/css/graphZoomNumberLineInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,pointFactory,axisFactory,axisZoom){"use strict"
;function createCanvas($container,config){var width=4+config.unitSize*(2+config.max-config.min);return scaleRaphael($(".shape-container",$container)[0],width,260)}function buildAxisConfig(rawConfig){var _default={top:60,left:50,unitSize:50,min:0,max:10,unitSubDivision:2,arrows:!0,point:{color:"#266d9c"}};return _.merge(_default,{min:void 0===rawConfig.min?void 0:parseInt(rawConfig.min),max:void 0===rawConfig.max?void 0:parseInt(rawConfig.max),unitSubDivision:void 0===rawConfig.unitSubDivision?void 0:parseInt(rawConfig.unitSubDivision),point:{color:rawConfig.graphColor}})}function _format(num){var str=num+""
;return str.length>5&&(str.match(/99999\d$/)?str=Math.round(1e4*num)/1e4:(str=Math.floor(1e3*num)/1e3,str=str.toString(),str.match(/^\d+\.\d{3,}$/)&&(str+="..."))),str}var graphZoomNumberLineInteraction={getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties,config.assetManager),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var value=this.getRawResponse();return value?{base:{float:value}}:{
base:null}},destroy:function(){$(this.dom).off().empty()},getTypeIdentifier:function(){return"graphZoomNumberLineInteraction"},initialize:function(id,dom,config){function findRect(allRects,position){var ret,positions=_.pluck(allRects,"position"),stepWidth=positions[1]-positions[0];return _.each(allRects,function(rect){var pos=parseInt(rect.position);if(pos<position&&position<pos+stepWidth)return ret=rect,!1}),ret}function zoomOnRect(x){var rect=findRect(rects,x);if(rect){selectedRect&&selectedRect.rect.hide(),zoomEffect&&zoomEffect.remove(),selectedRect=rect,zoomAxis.addCssClass(selectedRect.rect,"scene scene-selected-rectangle"),selectedRect.rect.show(),
zoomAxis.setConfig("labels",[_format(selectedRect.coord),_format(selectedRect.coord+subDivisionIncrement)]),zoomAxis.render();var containerBox=zoomAxis.buildContainerBox({shadow:!0});zoomAxis.addCssClass(containerBox,"scene scene-zoom-popup");var left=getSelectedPointPositionLeft();void 0!==left&&drawZoomAxisPoint(left),zoomAxis.clickable().click(function(e){drawZoomAxisPoint(e.layerX),updateAxisPoint()}),zoomEffect=axisZoom.drawZoomEffect(paper,selectedRect.rect,containerBox),zoomAxis.addCssClass(zoomEffect,"scene scene-zoom-effect")}}function drawAxisPoint(coord){axisPoint&&axisPoint.remove();var top=axis.getOriginPosition().top,pointConfig={radius:6,
removable:!1,glow:!1};return pointConfig=_.defaults(pointConfig,_this.axisConfig.point),axisPoint=pointFactory(paper,axis,pointConfig),axisPoint.setCartesianCoord(coord,top,pointConfig),axisPoint.render(),axisPoint.children.attr({cursor:"default"}),axis.getSet().push(axisPoint.children),axisPoint}function drawZoomAxisPoint(left){zoomPoint&&zoomPoint.remove();var zoomAxisConfig=zoomAxis.getConfig(),pointConfig={x:left,axis:"x",xMin:zoomAxisConfig.left,xMax:zoomAxisConfig.left+zoomAxisConfig.unitSize,removable:!1,on:{dragStop:updateAxisPoint}};return pointConfig=_.defaults(pointConfig,_this.axisConfig.point),zoomPoint=pointFactory(paper,zoomAxis,pointConfig),
zoomPoint.render(),zoomPoint.drag(),zoomAxis.getSet().push(zoomPoint.children),zoomPoint}function getZoomPointCoordinate(){return selectedRect.coord+zoomPoint.getCartesianCoord().x/_this.axisConfig.unitSubDivision}function getSelectedPointPositionLeft(){if(selectedCoord){var zoomAxisConfig=zoomAxis.getConfig(),offset=zoomAxisConfig.unitSize*_this.axisConfig.unitSubDivision/(10*_this.axisConfig.unitSubDivision),left=(selectedCoord-selectedRect.coord)*zoomAxisConfig.unitSize*_this.axisConfig.unitSubDivision+offset;if(left=parseInt(_format(left)),zoomAxisConfig.left<=left+offset&&left-offset<=zoomAxisConfig.left+zoomAxisConfig.unitSize)return left}}
function updateAxisPoint(){selectedCoord=getZoomPointCoordinate(),drawAxisPoint(selectedCoord),_.defer(function(){_this.trigger("responseChange")})}function initAxis($canvasContainer,axisConfig){var axisSizePx,zoomAxisConfig;subDivisionIncrement=1/axisConfig.unitSubDivision,paper=createCanvas($canvasContainer,axisConfig),axis=new axisFactory(paper,axisConfig),axisSizePx=(axisConfig.max-axisConfig.min)*axisConfig.unitSize,zoomAxisConfig=_.defaults({min:0,max:1,top:axisConfig.top+150,unitSize:axisSizePx,unitSubDivision:10,arrows:!1,labels:["A","B"]},axisConfig),zoomAxis=new axisFactory(paper,zoomAxisConfig),zoomAxis.getSet().hide(),
rects=axisZoom.createZoomable(axis),axis.clickable().click(function(e){zoomOnRect(e.layerX)})}function reset(){selectedRect&&(selectedRect.rect.hide(),selectedRect=void 0),zoomEffect&&(zoomEffect.remove(),zoomEffect=void 0),zoomAxis&&zoomAxis.getSet().remove().clear(),axisPoint&&(axisPoint.remove(),axisPoint=void 0),selectedCoord=void 0}function setAxis(axisConfig){_this.axisConfig=buildAxisConfig(_.merge(_this.config,_this.axisConfig,axisConfig)),initAxis($container,_this.axisConfig),reset()}event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{}
;var paper,axis,zoomAxis,zoomEffect,selectedRect,selectedCoord,subDivisionIncrement,axisPoint,zoomPoint,rects,$container=$(dom),_this=this;this.reset=function(){reset()},this.getRawResponse=function(){return selectedCoord},this.setRawResponse=function(selected){var axisSelectedX,zoomAxisSelectedX;selectedCoord=selected,axisSelectedX=axis.coordinateToPosition(selected).left+1,zoomOnRect(axisSelectedX),zoomAxisSelectedX=getSelectedPointPositionLeft(),drawZoomAxisPoint(zoomAxisSelectedX),updateAxisPoint()},this.axisConfig=buildAxisConfig(this.config),initAxis($container,this.axisConfig),this.on("axischange",setAxis)},setResponse:function(response){
